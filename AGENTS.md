...
- **영업 담당자 포털(user-web)**: 대시보드 · 거래처 · 영업 기회 · AI 문서 생성 · 웹 에디터 · 문서 보관함(표준 양식·품목 카탈로그·내 문서함) · 메일(발송 이력·수신함·연동·템플릿) · 설정(사이드바 맨 아래 메뉴: 계정 정보·회사 정보·보안)
...
- **`설정` 묶음은 없다 — 설정은 사이드바 맨 아래 프로필 메뉴다** (설정 7). 담겨 있던 둘이
  ...
  프로필 줄은 링크가 아니라 **메뉴**다(`계정 정보`·`회사 정보`·`보안` → `?tab=`) — 링크 하나로
  두면 나머지 둘은 화면에 들어간 뒤 탭을 눌러야만 닿아 사이드바만 봐서는 있는 줄도 모른다.
  메뉴는 **위로 열고**(`side="top"`) 폭을 트리거에 맞춰 사이드바 안에 둔다
  (`w-(--radix-dropdown-menu-trigger-width)`), 트리거의 화살표도 **위를 가리킨다** —
  아래를 가리키면 거짓말이다. 열려 있는 탭에는 `aria-current` + 굵기로 표시를 남긴다
  (다시 열었을 때 어디에 있는지 알아야 한다 · 색만 쓰지 않는다, ACC_*).
  머리글(`DropdownMenuLabel`)은 두지 않는다 — 바로 아래 트리거가 이미 `설정` 이라고 적는다.
