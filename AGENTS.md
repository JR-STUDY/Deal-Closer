...
- **수주(F-115)는 저장을 막지 않는다.** 수주는 이미 확정된 사실이고 다음 기회 생성은 선택이므로,
  전이는 그대로 저장하고 응답의 `promptNextOpportunity` 신호로 팝업을 띄운다 — 팝업을 먼저
  띄워 저장을 붙잡으면 취소했을 때 수주 자체가 안 된다. 후속 기회 생성 경로는
  `POST /api/opportunities/:id/next` → `createOpportunity()` **하나뿐**이고, 새 기회는
  **INITIAL·₩0** 에서 시작한다 — 예상 금액을 복제하지 않는다(확정 문서가 정하는 값이다).
  기본 이름(`거래처명 N차 도입`)은 서버 순수 함수가 정해 화면마다 다른 제안이 뜨지 않게 한다.
- **수주를 되돌릴 때 다음 기회는 확인창에서 처리를 묻는다.** 확인창은 결과를 미리 말한다 —
  어떤 기회가 연결돼 있고 거기 문서가 몇 건인지 밝히고 `함께 삭제`(기본 **꺼짐**)를 받는다.
  체크하지 않으면 다음 기회는 남고 `previousOpportunityId` 연결만 끊는다.
  **삭제해도 문서는 지우지 않는다 — 연결만 끊어 보관함으로 되돌린다.** 문서는 사람이 만든
  산출물이라 단계를 되돌린 부작용으로 조용히 사라지면 안 된다. 삭제·연결 해제는 단계 되돌리기와
  **한 트랜잭션**이다.
