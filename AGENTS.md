...
- **영업 담당자 포털(user-web)**: 대시보드 · 거래처 · 영업 기회 · AI 문서 생성 · 웹 에디터 · 문서 보관함 · 메일(발송 이력·수신함·연동·템플릿) · 설정(회사 정보 · 품목 카탈로그) · 내 계정(사이드바 맨 아래)
...
- **프로필 화면의 두 진입점은 서로 다른 탭을 가리킨다** (설정 7). 사이드바 맨 아래 프로필 줄은
  **`내 계정`**(→ `계정 정보`), `설정` 묶음은 **`회사 정보`**(→ `회사 정보`) 다. 예전에는 둘 다
  `/settings/profile` 로 가서 **이름만 다른 같은 링크가 사이드바에 두 개** 있었다(사용자 피드백).
  그 화면은 애초에 `개인이냐 회사냐`로 탭이 갈려 있으니, 문마다 자기 탭을 열게 하면 이름과
  목적지가 맞는다. 강조는 **정확히 한 곳만** 켠다 — 아래 줄은 회사 탭이 아닐 때, 설정 묶음의
  `회사 정보` 는 회사 탭일 때. 탭을 보지 않으면 한 화면에서 두 곳이 함께 켜져 갈라 놓은 두
  진입점이 다시 같은 것처럼 보인다. 아래 줄의 둘째 칸은 **역할이 아니라 `내 계정`** 이다 —
  역할(`영업 담당자`)은 위 브랜드 줄의 `영업 담당자 포털` 과 겹쳐 읽혔고 정작 이 줄이 하는
  일은 어디에도 적혀 있지 않았다(역할은 문 안쪽 프로필 히어로의 배지에 그대로 있다).
  그래서 `AppSidebar` 는 `roleLabel` 을 **받지 않는다** — 쓰지 않는 값을 받으면 호출측이
  매번 계산해 넘긴다.
...
- **설정 값은 "개인이냐 회사냐" 로 나눈다** (설정 7). 직함·연락처는 사람마다 다르므로 `User`
  ...
  **열린 탭은 주소(`?tab=`)에 있다** — 목록 정렬·페이지·캘린더의 달과 같은 규칙이고, 그래야
  사이드바의 두 진입점이 각자의 탭을 지목할 수 있다. `ProfileTabs` 는 **제어 컴포넌트**다
  (`defaultValue` 로 두면 이미 그 화면에 있을 때 다른 진입점을 눌러도 주소만 바뀌고 탭은
  그대로다 — 같은 라우트라 다시 마운트되지 않는다). 탭을 바꿀 때는 `router.replace` 가 아니라
