...
- **영업 담당자 포털(user-web)**: 대시보드 · 거래처 · 영업 기회(목록·칸반 · 마감 캘린더) · AI 문서 생성 · 웹 에디터 · 문서 보관함(표준 양식·품목 카탈로그·내 문서함) · 메일(발송 이력·수신함·연동·템플릿) · 설정(사이드바 맨 아래 프로필 줄 → 계정 정보·회사 정보·보안 탭)
...
pnpm test:document-version   # 버전 묶음 유틸·연결 후보 where 조건 검증 (DB 없이 실행)
...
src/
  ...
  components/
    ...
    opportunity/       # 기회 공용 — 등록 버튼·폼 다이얼로그(목록 전용), 단계 흐름 안내,
                       ...
                       #   opportunity-calendar(월 캘린더 — 대시보드 카드와 전용 페이지가 함께 쓴다:
                     #     `density="compact"` 면 결말 아이콘만, 기본 `full` 이면 기회명·금액),
  ...
  lib/
    ...
    opportunity-calendar.ts # 캘린더가 그릴 데이터를 읽는 **단 하나의 조회** (server-only, F-302) —
                         #   범위는 그리드 구간 · 합계는 그 달. 대시보드 카드와 전용 페이지가 공유
    calendar.ts          # 월 캘린더 순수 함수 (F-302) — month 쿼리 파싱·달 이동 주소 · CALENDAR_HREF ·
...
- **월 캘린더는 두 자리에 있고 컴포넌트·조회는 하나다** (F-302). 대시보드 카드는
  **컴팩트**(`density="compact"` — 칸에 결말 아이콘만, 칸 높이 48px)이고, 전용 페이지
  `/opportunities/calendar`(`CALENDAR_HREF`, 사이드바 `영업 기회` 묶음의 하위 항목)는
  **full**(기회명·금액·접기)이다. 시연 데이터처럼 하루 1~3건이 쌓이면 full 캘린더가 대시보드
  한 화면을 다 먹어 KPI·차트가 스크롤 아래로 밀렸다(실측 카드 높이 1100px → 컴팩트 479px).
  **그래도 컴포넌트를 둘로 나누지 않는다** — 갈리는 것은 칸 내용뿐이고 격자·요일 머리글·주말
  색·월 이동·금액 타일은 완전히 같다(라벨/값 2열 표를 렌더러 하나로 둔 것과 같은 판단).
  **full 칸의 일정도 한 줄이다**(아이콘 + 기회명). 한때 두 줄(이름 / 금액 + 결말)이라 링크
  하나가 43px 이었고, 두 건이 든 칸이 152px 이 되어 **1440×900 에 달이 통째로 들어가지
  않았다**(문서 1,148px). 달력의 쓸모가 "달 전체를 한 눈에" 인데 그걸 잃은 것이다 —
  지금은 900px 로 딱 들어간다. 칸에서 뺀 금액·거래처·결말은 **툴팁**이 받는다.
  컴팩트 칸은 이름까지 감추므로 세 가지를 지킨다: 결말을 **아이콘 모양**으로 구분하고(색만으로는
  단서가 아예 없다 — ACC_*), 링크의 `aria-label` 에 **기회명·금액·결말**을 모두 담으며,
  누를 자리를 아이콘(12px)보다 큰 20px 로 둔다. 감춘 건수(`+N`)는 링크가 아니다 — 더 볼 곳은
  헤더의 `전체 보기`(대시보드에만 있다)다. 칸 상한은 밀도마다 상수 하나씩이다
  (`DAY_EVENT_LIMIT` · `COMPACT_DAY_EVENT_LIMIT`) — 화면이 `slice` 를 직접 하면 어떤 날은
  2개, 어떤 날은 3개가 보인다.
- **캘린더 표식의 툴팁은 `title` 이 아니라 Radix 다** (`calendar-event-tooltip.tsx`).
  `title` 은 뜨기까지 1초 가까이 걸리고 **키보드 초점에는 아예 뜨지 않으며** 줄바꿈이
  제어되지 않는다. 컴팩트 칸은 아이콘 하나가 링크의 전부라 "누르면 어디로 가는가" 를 알
  방법이 툴팁뿐인데 그게 가장 부실했다. 툴팁은 **기회명 · 거래처 · 금액 · 결말**을 담고,
  거래처명이 특히 중요하다 — 기회명은 `그룹웨어 구축` 처럼 거래처가 달라도 같은 문구가
  반복되므로 회사명이 없으면 두 표식이 같은 건인지 알 수 없다(`CalendarEvent.accountName`
  은 **옵셔널**이라 안 넘겨도 캘린더는 그려진다). 툴팁은 **보조 설명이지 접근성 이름이
  아니다** — 링크는 여전히 `aria-label` 로 스스로를 말한다.
  **담는 내용은 밀도마다 다르다** — 칸에 이미 보이는 것을 툴팁이 되풀이하지 않는다.
  대시보드(컴팩트)는 **거래처·기회명 두 줄**로 끝낸다(결말은 아이콘 모양이 이미 말하고 월
  단위 금액은 헤더 합계 타일이 맡는다). 전용 페이지는 금액·결말 줄을 더한다. 그래서
  `amountLabel`·`outcomeLabel` 은 옵셔널이고 **둘이 함께 있을 때만** 그 줄을 그린다.
  배치는 **소속이 위, 값이 아래**다 — 상세 브레드크럼(`거래처 > 회사명 > 기회명`)과 같은
  순서라 사용자가 새로 배우지 않는다.
- **반전된 판(툴팁) 위에서는 `muted-foreground` 를 쓰지 않는다.** `TooltipContent` 는
  `bg-foreground` + `text-background` 라 본문과 색이 뒤집힌 판이고, 그 위의
  `text-muted-foreground` 는 **밝은 배경을 전제로 만든 회색**이라 거의 읽히지 않는다
  (실측: 거래처명이 그 상태였다). 흐린 글자가 필요하면 **판 색을 투명도로 낮춘다**
  — `text-background/75` 면 라이트 11.1:1 · 다크 9:1 로 양쪽 모두 AAA(7:1)를 넘는다.
  같은 이유로 색 대비를 눈이 아니라 **실측으로** 확인한다: 이 프로젝트의 토큰은 `oklch`·
  `lab` 이라 `getComputedStyle` 문자열을 rgb 로 파싱하면 엉뚱한 값이 나온다(canvas 로
  1×1 픽셀을 래스터화해 재면 실제 sRGB 값을 얻는다).
  **Provider 는 표식마다 두지 않고 표 하나를 감싼다** — 표식이 100개를 넘어 컨텍스트가 그만큼
  생기고, 옆 칸으로 옮길 때 지연 없이 이어 열리는 그룹 동작을 잃는다(`InfoHint` 가 스스로
  Provider 를 감싸는 것은 화면에 한둘뿐이라서다 — 사정이 다르다).
  **조회도 하나다** — `@/lib/opportunity-calendar` 의 `loadOpportunityCalendar()`. 두 화면이
  각자 조회를 적으면 다른 범위·다른 필드를 보게 되고, 같은 달의 같은 캘린더가 서로 다른 그림을
  그린다. 그 함수가 못박는 둘: **범위는 그 달이 아니라 그리드 구간**이고(그 달만 읽으면 같은
  주에 걸친 앞뒤 달 칸이 늘 비어 보인다) **합계는 그리드가 아니라 그 달** 기준이다(그리드로
  합치면 앞뒤 달 금액이 섞인다).
- **캘린더 계산은 `@/lib/calendar` 순수 함수를 쓴다** (F-302). 올리는 일정은 **기회의 예상
  마감일(`expectedCloseDate`) 하나**다
  ...
  진행 중·수주·실주를 아이콘 **모양**과 라벨(실주는 취소선)로 구분한다. 카드 헤더의 큰 숫자는
  **확정 금액**(그 달 수주 합)과 **전체 기회 금액**(진행 중 + 수주 + **실주**)이고 **한 숫자로
  합치지 않는다** — "된 돈"과 "그 달에 걸려 있던 판의 크기"는 다른 사실이다. 실주를 전체에 넣는
  이유는 그것도 그 달에 실제로 다룬 건이기 때문이며(진행 중만 세면 달을 넘길 때마다 총량이 조용히
  줄어드는 것처럼 보였다), 포함 여부는 숫자만 보고 알 수 없으므로 **라벨에 `실주 포함` 을 적는다**.
  진행 중 합(`expectedAmount`)은 버리지 않고 전체 타일의 보조 줄에 남긴다. `monthRevenue` 는
  세 결말의 금액·건수를 **각각** 들고 있는다 — 화면이 `total - confirmed` 로 되짚으면 진행 중과
  실주를 가릴 수 없다. 이 합계는 **기회 기준**이라 상단 KPI 의 계약 매출(**문서** 기준 누적)과
  다른 값이 정상이다(같게 맞추려 들지 말 것 — 카드 설명은 세로 공간을 아끼려고 걷어냈으니
  이 문단이 그 근거다). 월 구간 판정은 `@/lib/pipeline` 의 `monthRange`
  ·`isWithinRange` 를 재사용한다 — 같은 "그 달" 을 두 모듈이 각자 계산하면 하루 차이로 갈린다.
  **월 이동은 카드 헤더 가운데**다 — `grid-cols-[1fr_auto_1fr]` 로 양쪽 여백을 똑같이 나눈다
  (`flex` + `mx-auto` 는 제목 길이가 중심을 밀어 위치가 흔들린다).
