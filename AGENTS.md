...
- **월 캘린더는 두 자리에 있고 컴포넌트·조회는 하나다** (F-302). 대시보드 카드는
  ...
  **그래도 컴포넌트를 둘로 나누지 않는다** — 갈리는 것은 칸 내용뿐이고 격자·요일 머리글·주말
  ...
  컴팩트 칸은 기회명을 감추므로 두 가지를 지킨다: 결말을 **아이콘 모양**으로 구분하고(색만으로는
  단서가 아예 없다 — ACC_*), 링크의 `aria-label` 에 **기회명·금액·결말**을 모두 담는다.
  누를 자리는 아이콘(12px)보다 큰 20px 다. 감춘 건수(`+N`)는 링크가 아니다 — 더 볼 곳은
