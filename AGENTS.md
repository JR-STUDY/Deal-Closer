...
- **문서 생성에 넣는 기회·거래처는 CRM 값 하나뿐이다** (F-211 · F-212). 생성 화면은 진행 중
  ...
  그 사실을 표시한다.
- **문서 생성 화면(`/generator`)은 한 화면 · 네 섹션이다** — 위저드로 나누지 않는다.
  네 값이 서로를 바꾸므로(양식 → 문서 종류 → 확정 견적서 노출) 함께 보여야 한다.
  묶음은 질문 단위다: ① 무엇을 만드는가(방식·문서 종류) ② 누구에게(기회 → 거래처)
  ③ 무엇을 보고(확정 견적서·첨부·참고 문서) ④ 무엇을 지시하는가(지시문·모델·생성).
  계위는 `_components/form-section.tsx` 하나가 정한다(번호·제목·설명·들여쓴 본문) —
  섹션마다 크기·여백을 따로 적으면 한 곳을 손볼 때 나머지가 조용히 어긋난다.
  예전에는 기회·확정 견적서·거래처·AI 모델이 `추가 설정` 이라는 **한 이름** 아래 있었다.
  그 넷은 서로 다른 질문이라 어떤 값이 어떤 결과를 바꾸는지 화면으로 알 수 없었다.
