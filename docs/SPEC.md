...
| 문서 라이브러리 | `/library` | **내 문서함** — 상태 탭·검색·종류 필터·카드/목록 보기 |
...
> **MVP 확장 — 문서 보관함 강화**: 원본 기획서의 "상태 탭·검색"에 더해, 앱 사이드바의 "문서 보관함"을 **내 문서함**(`/library`)과 **공용문서함**(`/library/common`)으로 나눈다. ① 내 문서함은 상태 탭·검색·종류 필터·카드/목록 보기를 제공하고, ② 공용문서함은 팀 공용 기준 문서를 모아 본다. 문서는 둘 중 하나에만 속하며 카드 메뉴로 두 문서함 사이를 이동한다. 파티션은 `Document.isCommon` 으로 관리한다.
