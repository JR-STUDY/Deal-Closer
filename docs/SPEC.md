...
| 베이스 템플릿 | `/library/templates` | 재사용 표준 문서 저장·복제 (※향후 확장 — 미구현) |
...
> **MVP 확장 — 문서 보관함 강화**: 원본 기획서의 "상태 탭·검색"에 더해, 앱 사이드바의 "문서 보관함"을 **내 문서함**(`/library`)과 **공용문서함**(`/library/common`)으로 나눈다. ① 내 문서함은 상태 탭·검색·종류 필터·카드/목록 보기를 제공하고, ② 공용문서함은 팀 공용 기준 문서를 모아 본다. 문서는 둘 중 하나에만 속하며 카드 메뉴로 두 문서함 사이를 이동한다(파티션은 `Document.isCommon`). 각 문서함 아래에는 사이드바에서 **폴더**를 다단계로 관리한다: 접기/펼치기, 이름변경(더블클릭), 형제끼리 드래그 순서변경, '폴더 추가' 버튼(선택 폴더 하위에 "새 폴더" 즉시 생성 후 바로 편집), 카드 '폴더 이동'으로 문서 담기. 폴더는 `Folder.isCommon`(문서함)·`Folder.parentId`(중첩)로 관리한다.
