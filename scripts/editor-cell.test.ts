...
/*
 * 이미지 블록에는 칸이 없다 — `alt`·`role` 은 캔버스에 글자로 보이지 않는다
 * (값이 없을 때 보이는 것은 "이미지 없음" 자리표시자이지 `alt` 가 아니다).
 * 로고·인감 자리를 뜻하는 `role` 은 회사 정보를 채울 대상을 지목하는 **식별자**라
 * 사용자가 고칠 값이 아니다 — 고치면 인감 자리에 로고가 찍힌다.
 */
check(
  editableCells(
    block("image", {
  ...
  [],
  "이미지 블록에는 고칠 칸이 없다 (alt·role 은 인스펙터의 속성이다)",
);
